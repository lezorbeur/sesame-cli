...
- **`build_system(config)`**: Creates a `sesame.Builder` object from a dictionary. Supports 1D and 2D grids, multiple materials, doping, defects, and contacts.
- **`solve_equilibrium(tol, maxiter, htpy)`**: Finds the potential at zero bias.
- **`solve_at_voltage(voltage, tol, maxiter, htpy)`**: Solves the full drift-diffusion system at a specific bias.
- **`run_iv_curve(voltages, tol, maxiter, htpy)`**: Sweeps through a list of voltages and returns an array of currents.
- **`get_metrics()`**: Returns a dictionary with current density and potential extremes.
- **`export_csv(filename)`**: Exports the current solution mesh to a CSV file.
- **`save(filename)`** / **`load(filename)`**: Serializes/Deserializes the entire simulation state.
...
**Options:**
- `summary`: Path to the `sweep_summary.json` generated by `sweep.py`.
- `--x-param`: The parameter name (from the JSON config) to use for the X-axis.
- `--voltage`: The bias voltage at which to compare current densities.
- `--out`: Filename for the resulting PNG plot.

This command generates a plot showing how the current density at the specified voltage changes as one of the swept parameters is varied.
